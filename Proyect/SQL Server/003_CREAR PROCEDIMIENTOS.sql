...
begin try
	...
	declare @compra table(idusuario int,idproveedor int,idtienda int,totalcosto decimal(10,0))
	declare @detallecompra table(idcompra int,idproducto int,cantidad int,preciounidadcompra decimal(10,0),preciounidadventa decimal(10,0),totalcosto decimal(10,0))
	declare @tiendaproducto table(idtienda int,idproducto int,cantidad int,preciounidadcompra decimal(10,0),preciounidadventa decimal(10,0),totalcosto decimal(10,0))
	 ...
	 TotalCosto = Node.Data.value('(TotalCosto)[1]','decimal(10,0)')
	 ...
	 PrecioUnidadCompra = Node.Data.value('(PrecioUnidadCompra)[1]','decimal(10,0)'),
	 PrecioUnidadVenta = Node.Data.value('(PrecioUnidadVenta)[1]','decimal(10,0)'),
	 TotalCosto = Node.Data.value('(TotalCosto)[1]','decimal(10,0)')
...
CONVERT(decimal(10,0), c.TotalCosto)[TotalCosto],
...
CONVERT(decimal(10,0),dc.PrecioUnitarioCompra)PrecioUnitarioCompra,
CONVERT(decimal(10,0),dc.TotalCosto)[TotalCosto] 
...
begin try
	...
	declare @venta table (idtienda int,idusuario int,idcliente int default 0,tipodocumento varchar(50),cantidadproducto int,cantidadtotal int,totalcosto decimal(10,0),importerecibido decimal(10,0),importecambio decimal(10,0))
	declare @detalleventa table (idventa int,idproducto int,cantidad int,preciounidad decimal(10,0),importetotal decimal(10,0))
	...
	select 
		 ...
		 TotalCosto = Node.Data.value('(TotalCosto)[1]','decimal(10,0)'),
		 ImporteRecibido = Node.Data.value('(ImporteRecibido)[1]','decimal(10,0)'),
		 ImporteCambio = Node.Data.value('(ImporteCambio)[1]','decimal(10,0)')
	...
	insert into @detalleventa(idventa,idproducto,cantidad,preciounidad,importetotal)
		 ...
		 IdProducto = Node.Data.value('(IdProducto)[1]','int'),
		 Cantidad = Node.Data.value('(Cantidad)[1]','int'),
		 PrecioUnidad = Node.Data.value('(PrecioUnidad)[1]','decimal(10,0)'),
		 ImporteTotal = Node.Data.value('(ImporteTotal)[1]','decimal(10,0)')
...
CONVERT(decimal(10,0), V.TotalCosto)[TotalCosto],
CONVERT(decimal(10,0),V.ImporteRecibido)[ImporteRecibido],
CONVERT(decimal(10,0), V.ImporteCambio)[ImporteCambio],
...
CONVERT(decimal(10,0),dv.PrecioUnidad)[PrecioUnidad],
CONVERT(decimal(10,0),dv.ImporteTotal)[ImporteTotal] 
...
--PROCEDMIENTO PARA REPORTE TIENDA PRODUCTO
 ...
 pt.Stock[Stock en tienda],convert(decimal(10,0),pt.PrecioUnidadCompra)[Precio Compra],convert(decimal(10,0),pt.PrecioUnidadVenta)[Precio Venta]
 from PRODUCTO_TIENDA pt
 inner join PRODUCTO p on p.IdProducto = pt.IdProducto
 inner join TIENDA t on t.IdTienda = pt.IdTienda
...
Create proc sp_ReporteDashbord
as 
begin 
	
	select 
	(select COUNT(*) from PRODUCTO p inner join PRODUCTO_TIENDA t on p.Codigo = t.IdProducto ) [Codigo],
	(select COUNT(*) from CLIENTE) [IdCliente],
	(select COUNT(*) from VENTA) [IdVenta],
	(select COUNT(*) from CATEGORIA) [IdCategoria]

END
go

-- VENTAS DEL MES - DASHBOARD
CREATE proc sp_GraficosDashBoard
as
begin
	...
	set @fecha_minima = dateadd(MONTH, -5,@fecha_maxima);
	set @fecha_minima = dateadd(DAY,- (DAY(@fecha_minima)-1),@fecha_minima);

	select year(FechaRegistro)[anio],MONTH(FechaRegistro)[dia],DATENAME(MONTH,FechaRegistro)[mes],COUNT(*)[TotalCosto] from [VENTA]
...
--update VENTA set FechaRegistro = (dateadd(MONTH, -2, @fecha)) where IdVenta = 1
--select IdVenta,FechaRegistro from VENTA

--PRODUCTOS MAS VENDIDOS - DASHBOARD
create proc sp_GraficosDashBoard2
as
begin
	declare @fecha_maxima datetime
	declare @fecha_minima datetime
	...
	Select p.Nombre[Producto],count(*)[Cantidad] from VENTA v
	inner join DETALLE_VENTA d on v.IdVenta = d.IdVenta
	inner join PRODUCTO p on p.Codigo = d.IdProducto
...
-- STOCK DE PRODUCTOS - DASHBOARD
create proc sp_GraficosDashBoard3
as
begin
	declare @fecha_maxima datetime
	declare @fecha_minima datetime

	set @fecha_maxima = (select MAX(FechaRegistro) from PRODUCTO_TIENDA);

	set @fecha_minima = dateadd(MONTH, -30,@fecha_maxima);
	set @fecha_minima = dateadd(DAY,- (DAY(@fecha_minima)-1),@fecha_minima);

	Select p.Nombre,t.Stock from PRODUCTO p
	inner join PRODUCTO_TIENDA t on p.IdProducto = t.IdProducto
	where t.FechaRegistro between @fecha_minima and @fecha_maxima
	group by t.Stock, p.Nombre
	order by p.Nombre desc
end	
go

-- REPORTE DE HOME - DASHBOARD
create proc sp_ReporteResumenVenta
as 
begin

	set dateformat dmy;

	select c.Nombre[Cliente], c.Telefono[Telefono], p.Nombre[Producto], v.CantidadTotal[Cantidad], v.TotalCosto[Total], CONVERT(char(10),
	v.FechaRegistro,110)[Fecha], (u.Nombres + '' + u.Apellidos)[Vendedor], v.IdVenta[IDventa], v.TipoDocumento[Documento] from VENTA v
